...
module Initialization =
    ...
    let handleInitialize (lspClient: ILspClient)
        ...
        logger.LogInformation(
            "{serverName} is released under MIT license and is not affiliated with Microsoft Corp.; see https://github.com/CoolCoderSuper/visualbasic-language-server",
            serverName)
...
*)
    ...
    let handleInitialized (lspClient: ILspClient)
        ...
        async {
            ...
            | ex ->
                logger.LogWarning(
                    "handleInitialized: could not retrieve `vb` workspace configuration section: {error}",
                    ex |> string
                )
